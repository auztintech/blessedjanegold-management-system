...
import type { SalesPersonLowStockProduct } from "@/types/dashboard";

interface SalesPersonLowStockProps {
  data: SalesPersonLowStockProduct[];
}

export function SalesPersonLowStock({ data }: SalesPersonLowStockProps) {
  if (!data.length) {
    return (
            ...
            return (
              <tr
                ...
                <td className="px-3 py-3 text-right font-semibold text-red-500">
                  {product.total_quantity.toLocaleString("en-NG")}
                </td>

                <td className="px-3 py-3 text-right text-gray-600">
                  {product.low_stock_threshold.toLocaleString("en-NG")}
                </td>
