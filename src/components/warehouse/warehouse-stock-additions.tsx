...
import type { WarehouseStockAddition } from "@/types/dashboard";

interface WarehouseStockAdditionsProps {
  data: WarehouseStockAddition[];
}

export function WarehouseStockAdditions({
  data,
}: WarehouseStockAdditionsProps) {
  if (!data.length) {
    return (
      <div className="flex min-h-30 items-center justify-center rounded-lg bg-gray-50 px-4">
        <p className="text-sm text-gray-400">No recent stock additions.</p>
      </div>
    );
  }
