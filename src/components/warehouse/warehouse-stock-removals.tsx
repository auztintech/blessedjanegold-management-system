...
import type { WarehouseStockRemoval } from "@/types/dashboard";

interface WarehouseStockRemovalsProps {
  data: WarehouseStockRemoval[];
}

export function WarehouseStockRemovals({ data }: WarehouseStockRemovalsProps) {
  if (!data.length) {
    return (
      <div className="flex min-h-30 items-center justify-center rounded-lg bg-gray-50 px-4">
        <p className="text-sm text-gray-400">No recent stock removals.</p>
      </div>
    );
  }
        ...
        <tbody className="divide-y divide-gray-100">
          {data.map((item) => (
            <tr key={item.id} className="transition-colors hover:bg-gray-50">
              ...
              <td className="px-3 py-3 text-gray-600">{item.reason || "—"}</td>
